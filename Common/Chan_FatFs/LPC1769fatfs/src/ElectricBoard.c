...
void GPIO_Init(void)
{
	//	LPC_PINCON->PINSEL3 |= (3<<24);//GPIO0 pin function select register
	LPC_PINCON->PINSEL4 &= ~(0x0f<<8);//PIO2_4 and PIO2_5 GPIO mode

	//	LPC_PINCON->PINMODE3  |=  (1<<24);//GPIO0 pin mode select register
	LPC_PINCON->PINMODE4 |= (0x0a<<8);//PIO2_4 and PIO2_5 neither pullup nor pulldown

	//GPIO0~4 pin open drain mode select register 0:disable open drain 1:enable open drain
	//ex)LPC_PINCON->PINMODE_OD2 &= ~(1<<12);
	LPC_PINCON->PINMODE_OD2 &= ~(3<<4);//PIO2_4 and PIO2_5  no open drain

	//permit to access GPIO0~4(default: all GPIO can access) 0:can access GPIOx 1:cannot access GPIOx
	//ex)LPC_GPIO0->FIOMASK = 0x0000000;

	//set GPIO0~4 direction 0:input 1:output
	//ex)LPC_GPIO0->FIODIR |= (1<<12);
	LPC_GPIO2->FIODIR |= (3<<4);//PIO2_4 and PIO2_5 output mode
