...
#define SELECT()	LPC_GPIO0->DATA &= ~(1<<11)		/* CS = L */
#define	DESELECT()	LPC_GPIO0->DATA |= (1<<11)		/* CS = H */
#define	MMC_SEL		!(LPC_GPIO0->DATA &(1<<11))		/* CS status (true:CS == L) */
#define	FORWARD(d)	Putc(d)		/* Data forwarding function (Console out in this example) */
...
{
	UINT n = 14745600UL/ 100000;
	do {
		LPC_GPIO0->DATA;LPC_GPIO0->DATA;LPC_GPIO0->DATA;LPC_GPIO0->DATA;LPC_GPIO0->DATA;LPC_GPIO0->DATA;
	} while (--n);
}
